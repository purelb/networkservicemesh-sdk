...
func resolveDomain(ctx context.Context, service, domain string, r Resolver) (url *url.URL, err error) {
	defer func() {
		if err == nil {
			log.FromContext(ctx).Debugf("Resolved url: %v", url)
		}
	}()
...
// undefined.
func formatURL(ip string, port string) (*url.URL, error) {
